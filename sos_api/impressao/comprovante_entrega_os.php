...
if (isset($id_ordem_recebida) && is_numeric($id_ordem_recebida)) {
    ...
    $pdf->Cell(0, 10, 'Comprovante entrega de Ordem de Serviço', 0, 1, 'C');
    ...
    $data_conclusao = $dados_ordem_servico['concluida_em'];
    $data_conclusao = substr($data_conclusao, 8, 2) . '/' . substr($data_conclusao, 5, 2) . '/' . substr($data_conclusao, 0, 4) . ' ' . substr($data_conclusao, 10, 9);
    $pdf->SetFont('Arial', '', 10);
    $pdf->Cell(0, 5, 'Data entrada: ' . $data_entrada, 0, 1);
    $pdf->Cell(0, 5, 'Data conclusão: ' . $data_conclusao, 0, 1);
    ...
    $pdf->Ln(3);
    $pdf->SetFont('Arial', 'B', 14);
    $pdf->Cell(0, 5, 'Serviço executado:', 0, 1);
    $pdf->SetFont('Arial', '', 10);
    $pdf->MultiCell(0, 5, $dados_ordem_servico['servico_executado'], 'B', 1);
    $pdf->MultiCell(0, 5, "Obs tecnico:" . $dados_ordem_servico['obs_tecnico'], 0, 1);
    // MATERIAIS
    ...
    $pdf->SetFont('Arial', 'B', 10);
    $pdf->Cell(0, 5, 'Total Ordem de Serviço: R$ ' . number_format($dados_ordem_servico['total_os'], 2, ',', '.'), 'B', 1);

    $pdf->Ln(10);

    $pdf->SetFont('Arial', 'B', 10);
    $pdf->Cell(0, 10, 'Garantia serviço: __________________', 0, 1);
    $pdf->Cell(0, 10, 'Garantia material: __________________', 0, 1);
    
    $pdf->Ln(10);
    $pdf->SetFont('Arial', '', 10);
    $pdf->Cell(0, 5, '_____________________________________________', 0, 1, 'C');
    $pdf->SetFont('Arial', 'B', 10);
    $pdf->Cell(0, 5, $dados_ordem_servico['nome'], 0, 1, 'C');
    $pdf->Cell(0, 5, date("d/m/Y H:i:s"), 0, 1, 'C');
    $pdf->Ln(10);
    $pdf->SetFont('Arial', '', 10);
    $pdf->Cell(0, 5, '_____________________________________________', 0, 1, 'C');
    $pdf->SetFont('Arial', 'B', 10);
    $pdf->Cell(0, 5, $dados_loja['nome_fantasia'], 0, 1, 'C');
